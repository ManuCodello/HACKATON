...
contenedor.innerHTML = `
    ...
    <div class="mt-4 text-center">
        <form action="/eliminar-receta/${receta.id}" method="post">
            <button class="text-red-500 hover:text-red-700 text-lg font-semibold transition duration-300" title="Eliminar">
...
document.getElementById("btn-solicitar-receta").addEventListener("click", async () => {
 ...
 document.getElementById("btn-solicitar-receta").addEventListener("click", async () => {
    try {
        ...
        document.getElementById("loader").style.display = "block";
        document.getElementById("botones-recetas").style.display = "none";
        
        // Hacer la solicitud para obtener la receta (aquí se asume que el endpoint existe)
        const response = await fetch("/generar-receta-inteligente", {
            method: "POST",
            headers: { "Content-Type": "application/json" }
        });
        ...
        document.getElementById("loader").style.display = "none";
        document.getElementById("botones-recetas").style.display = "flex";
        
        if (response.ok) {
            // Mostrar la receta generada o el resultado en el div de resultado
            document.getElementById("resultado").innerHTML = `
                <p>Receta generada: ${data.receta.nombre_receta}</p>
                <!-- Aquí puedes agregar más contenido según lo que retorne la receta -->
            `;
        } else {
            alert(data.error || "Ocurrió un error.");
        }
    } catch (error) {
        // En caso de error, ocultar el loader y mostrar un mensaje de error
        document.getElementById("loader").style.display = "none";
        document.getElementById("botones-recetas").style.display = "flex";
        console.error("Error:", error);
        alert("No se pudo conectar con el servidor.");
