...
def eliminar_receta(id):
    receta = Recetas.query.filter_by(id=id).first()  # Busca el ingrediente por su id
    db.session.delete(receta)  # Elimina el ingrediente de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios
    return redirect("/")  # Redirige a la página principal
