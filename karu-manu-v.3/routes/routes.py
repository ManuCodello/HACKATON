...
def generar_receta_ia():
    ingredientes = obtener_nombres_ingredientes(Ingredientes)  # Obtiene los nombres de todos los ingredientes
    recetas_previas = obtener_nombres_recetas(Recetas)  # Obtiene los nombres de todas las recetas previas

    for _ in range(5):  # Intentar generar una receta hasta 5 veces
        prompt = armar_prompt(ingredientes, recetas_previas)  # Crea un prompt para la API externa
        response = llamar_api(prompt)  # Llama a la API externa para generar una receta

        if response.status_code != 200:
            return jsonify({"error": "Error al conectar con Magic Loops"}), 500  # Si la API no responde correctamente

        try:
            receta_data = json.loads(response.text.strip())  # Procesa la respuesta de la API como JSON
        except json.JSONDecodeError:
            return jsonify({"error": "Respuesta no es JSON válido"}), 500  # Maneja errores de formato JSON

        nombre_generado = receta_data.get("nombre_receta", "").strip().lower()  # Extrae el nombre de la receta generada

        if receta_similar(nombre_generado, recetas_previas):  # Verifica si la receta ya existe
            continue  # Si es similar a una receta previa, intenta nuevamente

        # Si la receta es nueva, la guarda en la base de datos
        nueva = Recetas(
        ...
        db.session.add(nueva)  # Agrega la receta generada a la base de datos
        db.session.commit()  # Guarda los cambios
        return jsonify({"mensaje": "Receta generada correctamente", "receta": receta_data})  # Retorna un mensaje de éxito

    return jsonify({"error": "No se pudo generar una receta nueva tras varios intentos"}), 400  # Error si no se pudo generar la receta
