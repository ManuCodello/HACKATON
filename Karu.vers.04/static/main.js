...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function mostrarReceta(receta) {
    ...
    if (typeof receta.ingredientes === "string") {
      ingredientesHTML = receta.ingredientes.split(';').map(ing => `<li>${ing.trim()}</li>`).join('');
    } else if (typeof receta.ingredientes === "object") {
      ingredientesHTML = Object.entries(receta.ingredientes)
    ...
    if (typeof receta.pasos === "string") {
      pasosHTML = receta.pasos
        .split('|')
        .map(p => `<li>${p.trim()}</li>`)
        .join('');
    } else if (typeof receta.pasos === "object") {
      pasosHTML = Object.entries(receta.pasos)
  ...
  function mostrarRecetaia(receta) {
    const contenedor = document.getElementById("detalle-receta");
    let ingredientesHTML = "";
...
document.getElementById("btn-solicitar-receta").addEventListener("click", async () => {
  try {
    ...
    if (response.ok) {
      ...
      // Creamos un resumen legible de la receta
      const resumen = `
